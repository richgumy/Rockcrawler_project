...
class Joystick:
    ...
    # Left stick X axis value scaled between -255 (left) and 255 (right) with deadzone tolerance correction
    def leftX(self,deadzone=4000):
        self.refresh()
        raw = int(self.reading[3:9])
        return int(self.axisScale(raw,deadzone)*255)

    # Left stick Y axis value scaled between -255 (down) and 255 (up)
    def leftY(self,deadzone=4000):
        self.refresh()
        raw = int(self.reading[13:19])
        return int(self.axisScale(raw,deadzone)*255)

    # Right stick X axis value scaled between -255 (left) and 255 (right)
    def rightX(self,deadzone=4000):
        self.refresh()
        raw = int(self.reading[24:30])
        return int(self.axisScale(raw,deadzone)*255)

    # Right stick Y axis value scaled between -255 (down) and 255 (up)
    ...
    # Left Trigger value scaled between 0 to 255
    def leftTrigger(self):
        self.refresh()
        return int(self.reading[129:132])

    # Right trigger value scaled between 0 to 255
